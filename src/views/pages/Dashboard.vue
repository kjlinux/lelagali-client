...
const maxPrice = computed(() => {
    if (menus.value.length === 0) return 50000;
    return Math.max(...menus.value.map(m => m.price));
});

const minPrice = computed(() => {
    if (menus.value.length === 0) return 0;
    return Math.min(...menus.value.map(m => m.price));
});
...
const filteredMenus = computed(() => {
    return menus.value.filter((menu) => {
        ...
        return matchesSearch && matchesQuartier && matchesPrice;
    });
});
...
// Fonctions de gestion du panier dans localStorage
const CART_STORAGE_KEY = 'lelagali_cart';
...
// Surveiller les changements du panier et sauvegarder automatiquement
watch(cartItems, () => {
    saveCartToStorage();
}, { deep: true });
...
const handleSearch = (query) => {
    ...
    // Debounce pour éviter trop de filtres
...
const handlePaymentSuccess = async (orderDetails) => {
    ...
    // Réinitialiser le panier et nettoyer le localStorage
    cartItems.value = [];
    clearCartStorage();
...
onMounted(async () => {
    ...
    // 4. Charger les commandes si l'utilisateur est connecté
    if (user.value) {
        await loadOrders();
    }
});
    ...
    <div id="app" class="min-h-screen bg-[#FDF6EC] flex flex-col">
        ...
        <main class="flex-1">
            <div class="container mx-auto px-4 py-6">
                <MenuFilters @filter-change="handleFilterChange" :quartiers="availableQuartiers" :min-price="minPrice" :max-price="maxPrice" />
                    ...
                    <div class="mb-6" id="menu-section">
                        <div class="flex justify-between items-center mb-4">
                            <div>
                                ...
                                <div v-if="searchQuery" class="mt-2 flex items-center text-sm text-[#47A547]">
                                    <i class="pi pi-search mr-2"></i>
                                    <span>Recherche : "{{ searchQuery }}"</span>
                                    <button @click="searchQuery = ''" class="ml-2 text-[#E6782C] hover:underline">
                                        Effacer
                                    </button>
                                </div>
                            </div>
                    ...
                    <div v-if="totalFilteredMenus === 0" class="text-center py-12">
                        <div class="bg-white rounded-lg shadow-sm p-8 max-w-md mx-auto">
                            ...
                            <p class="text-gray-600 mb-4">
                                ...
                                <span v-else>Aucun menu ne correspond à vos critères.</span>
                                <br />Essayez de modifier vos critères de recherche ou vos filtres.
                            </p>
                            <button
                                @click="
                                    searchQuery = '';
                                    filters = { quartier: '', priceRange: [minPrice, maxPrice] };
                                "
                                class="bg-[#47A547] hover:bg-[#3d8b3d] text-white px-6 py-2 rounded-lg font-medium transition-colors"
                            >
